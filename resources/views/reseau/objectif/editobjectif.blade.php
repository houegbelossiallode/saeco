...
@section('section')
    <div class="page-titles">
        <div class="d-flex align-items-center">
            <h5 class="font-medium m-b-0">Modifier objectif</h5>
            <div class="custom-breadcrumb ml-auto">
                <a href="{{ route('home') }}" class="breadcrumb">Home</a>
                <a href="#!" class="breadcrumb">Modifier objectif</a>
            </div>
        </div>
    </div>
    <div class="container-fluid">
        <div class="card">
            <div class="card-content">
                <h5 class="card-title">Modifier objectif</h5>
                <form action="{{ route('objectif.update', $idobjectif) }}" method="post">
                    @method('post')
                    @csrf
                    <div class="row m-t-40">
                        <div class="input-field col s12 l6">
                            <input id="nbr" name="nbr" type="number"
                                class="validate @error('nbr') is-invalid @enderror"
                                value="{{ old('nbr') ?? $objectif->nombre }}">
                        ...
                        <div class="input-field col s12 l6">
                            <input id="ca" name="ca" type="number"
                                class="validate @error('ca') is-invalid @enderror" value="{{ old('ca') ?? $objectif->ca }}">
                    ...
                    <div class="row">
                        <div class="input-field col s12 l6">
                            <input id="debut" name="debut" type="date"
                                ...
                                value="{{ old('debut') ?? $objectif->debut }}">
                    ...
                    <div class="row">
                        <div class="input-field col s12 l6">
                            <select id="niveau" name="niveau" data-error=".errorTxt6"
                                ...
                                @foreach ($niveaux as $niveau)
                                    <option value="{{ $niveau->id }}" @if (old('niveau') == $niveau->id || $objectif->statut_id == $niveau->id) selected @endif>
                        ...
                        <div class="input-field col s12 l6">
                            <select id="type" name="type" data-error=".errorTxt6"
                                ...
                                <option value="Periodique" @if (old('type') == 'Periodique' || $objectif->type == 'Periodique') selected @endif>
                                    Periodique
                                </option>
                                <option value="Challenge" @if (old('type') == 'Challenge' || $objectif->type == 'Challenge') selected @endif>
                                    Challenge
                                </option>
                            </select>
                    ...
                    <div class="row" id="blocRecompense"
                        style="display: {{ $errors->has('recompense') || $objectif->type == 'Challenge' ? 'block' : 'none' }}">
                        <div class="input-field col s12">
                            <textarea id="recompense" name="recompense"
                                class="materialize-textarea validate @error('recompense') is-invalid @enderror">{{ old('recompense') ?? $objectif->recompense }}</textarea>
